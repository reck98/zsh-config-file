...
main() {
    ...
    local MODE="install"
    local CI_MODE=false
    local LOG_FILE="/tmp/vm_bootstrap.log"
    ...
    # Only check root for install/uninstall
    if [[ "$MODE" != "status" ]]; then
    ...
    local COMMON="curl wget git unzip zip tar htop ca-certificates"
    case "$PM" in
      apt)
